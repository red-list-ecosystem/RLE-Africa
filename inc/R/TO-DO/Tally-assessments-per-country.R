...
Africa %>% st_drop_geometry %>% 
  mutate(RLE_progress=
    case_when(
      NAME_EN %in% c("Seychelles","Mauritius","Kenya","Tanzania","Comores","Seychelles","Comoros") ~ "Subset of ecosystems",
      NAME_EN %in% c("Egypt","Mali","Mauritania") ~ "Strategic",
      !is.na(RLE_progress) ~ as.character(RLE_progress),
      TRUE ~ "None"
  )) %>% mutate(RLE_progress=factor(RLE_progress,levels=c("In progress","Preliminary/Rapid","Strategic","Subset of ecosystems","All ecosystems"))) %>% 
  filter(!is.na(RLE_progress)) %>% select(NAME_EN,RLE_progress) %>% arrange(RLE_progress) %>% print(n=100)
